...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate immediately based on returned user (more reliable than waiting for effects)
      const createdUser = await register(name, email, password);

      if (createdUser.role === "ADMIN") navigate("/admin", { replace: true });
      else navigate("/dashboard", { replace: true });
    } catch (err: unknown) {
      ...
      if (fieldErrs) {
        setFieldErrors({
          name: fieldErrs.name?.[0],
          email: fieldErrs.email?.[0],
          password: fieldErrs.password?.[0],
          confirmPassword: fieldErrs.confirmPassword?.[0],
        });
      }
    } finally {
      setSubmitting(false);
      ...
      <div className="w-full max-w-md md:max-w-lg bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg shadow-stone-900/10 border border-stone-200 p-6 md:p-8 space-y-5">
        <div className="space-y-1">
          ...
          <p className="text-xs md:text-sm text-stone-600">
            Save your details, view your bookings and enjoy a smoother stay at
            Eagle Villas.
          </p>
        </div>
